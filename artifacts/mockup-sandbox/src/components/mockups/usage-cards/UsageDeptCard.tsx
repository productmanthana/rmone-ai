...
const rows = [
  { name: "Unassigned",    count: 415, adoption: 40 },
  { name: "New York City", count: 55,  adoption: 33 },
  { name: "Cold Storage",  count: 5,   adoption: 60 },
  { name: "Mineola Field", count: 4,   adoption: 50 },
  { name: "Houston",       count: 3,   adoption: 33 },
  { name: "New Jersey",    count: 3,   adoption: 67 },
  { name: "Dallas",        count: 3,   adoption: 33 },
  { name: "Atlanta",       count: 2,   adoption: 0  },
];
const maxCount = Math.max(...rows.map(r => r.count));
const SHOW = 7;

export default function UsageDeptCard() {
  return (
    <div style={{ background: BG, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ width: 460, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 24px 14px", borderBottom: `1px solid ${BORDER}` }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: TEXT, letterSpacing: "-0.01em" }}>Usage by Department</div>
          <div style={{ fontSize: 11, color: MUTED, marginTop: 3 }}>Staff adoption rate by department — {rows.length} departments</div>
        </div>
              ...
              <div key={r.name} style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 24px" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ marginTop: 4, height: 5, borderRadius: 3, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
                    <div style={{ height: "100%", borderRadius: 3, width: `${barW}%`, background: TEAL }} />
                  </div>
                </div>
