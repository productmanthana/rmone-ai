...
const avgCycleDays = [
  { label: "Lead → Opp", days: 18, color: "var(--rm-accent-blue)" },
  { label: "Opp → Proposal", days: 34, color: "var(--rm-brand-lime)" },
...
function SectionLabel({ children, right, takeaway }: { children: React.ReactNode; right?: React.ReactNode; takeaway?: string }) {
  return (
    <div className="mb-2.5">
      ...
      {takeaway && <div className="mt-1 text-[11.5px] font-medium leading-snug" style={{ color: "rgba(255,255,255,0.72)" }}>{takeaway}</div>}
    </div>
  );
}
...
export default function ExecutiveAnalytics() {
  ...
  const totalRecords = 418;
  const maxDivCount = projectsByDivision[0].count;
  return (
    <div className="rmone-analytics min-h-screen relative overflow-hidden" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div className="relative p-6 max-w-[1440px] mx-auto">
        ...
        {/* HERO: backlog + gauges — chart 1 of 3 */}
        <div className="grid grid-cols-12 gap-4 mb-4">
            ...
            <div className="relative px-6 pt-5 pb-4 flex flex-col h-full">
              ...
              <div className="flex items-baseline gap-4 mt-1">
                ...
                <div className="text-[11px] leading-snug" style={{ color: "var(--rm-text-muted)" }}>
                  total value of approved contracts — the work we've won but not yet finished.<br />
                  12-month climb · Mar $128.4M → Feb $148.2M
                </div>
              </div>
          ...
          <Glass className="px-5 py-4" style={{ gridColumn: "span 4 / span 4" }}>
            <SectionLabel right="TTM" takeaway="We win 4 of every 10 pursuits we chase.">Command Gauges</SectionLabel>
            <div className="flex items-start justify-around mt-1">
              <Gauge pct={43.1} label="Win Rate" value="43.1%" sub="+2.4 pts QoQ" color="var(--rm-green)" size={122} />
              <Gauge pct={91.2} label="Forecast Acc." value="91.2%" sub="target >90%" color="var(--rm-accent-blue)" size={122} />
            </div>
            <div className="flex items-center justify-center">
              <Gauge pct={79.9} label="Schedule Health" value="79.9%" sub="171 of 214 on track" color="var(--rm-health-good)" size={112} />
            </div>
          </Glass>
        </div>
        ...
        {/* Plain-language coverage callouts */}
        <div className="grid grid-cols-4 gap-4 mb-4">
          {[
            { label: "Pipeline Capacity Coverage", value: "1.8×", plain: "We're chasing nearly twice the work our free capacity can absorb.", sub: "weighted pipeline vs available capacity · next 6 mo", pct: 90 },
            { label: "Book-to-Bill (TTM)", value: "1.12", plain: "For every $1 of work we finish, we win $1.12 of new work.", sub: "new awards vs revenue burned", pct: 74 },
            { label: "Backlog Coverage", value: "9.6 mo", plain: "We have 9.6 months of signed work in the bank.", sub: "backlog at current burn rate", pct: 80 },
            { label: "Forecast Accuracy", value: "91.2%", plain: "Revenue landed within 9% of what we planned.", sub: "planned vs landed revenue · target >90%", pct: 91 },
          ].map((s) => (
            <div key={s.label} className="relative rounded-xl px-4 py-3 overflow-hidden"
              ...
              <div className="text-[11px] mt-1.5 font-medium leading-snug" style={{ color: "rgba(255,255,255,0.72)" }}>{s.plain}</div>
              <div className="text-[9.5px] mt-0.5" style={{ color: "var(--rm-text-faint)" }}>{s.sub}</div>
            </div>
          ))}
        </div>

        {/* Funnel (step bars) + quarterly trend (chart 2) + division conversion (list) */}
        <div className="grid grid-cols-12 gap-4 mb-4">
          <Glass className="col-span-5 px-5 py-4">
            <SectionLabel right="TTM" takeaway="Of 312 leads this year, 81 became real projects.">Pipeline Conversion Funnel</SectionLabel>
              ...
              {funnelSteps.map((s, i) => (
                <div key={s.label}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-[30px] relative flex items-center justify-center">
          ...
          <Glass className="col-span-4 px-5 py-4 flex flex-col">
            <SectionLabel right="count by stage per quarter" takeaway="Awards keep climbing quarter over quarter.">Quarterly Funnel Trend</SectionLabel>
          ...
          <Glass className="col-span-3 px-5 py-4">
            <SectionLabel right="leads vs awarded" takeaway="Construction Mgmt turns the most leads into wins.">Division Conversion</SectionLabel>
        ...
        {/* Bottom band: status list · division list · win rate (chart 3) · schedule health */}
        <div className="grid grid-cols-12 gap-4 mb-4">
          <Glass className="col-span-3 px-5 py-4">
            <SectionLabel right="418 total records" takeaway="Over half of everything we track is live work.">Records by Status</SectionLabel>
            <div className="flex items-baseline gap-2 mb-3">
              <span className="text-[34px] font-extrabold tabular-nums leading-none" style={{ textShadow: "0 0 18px rgba(107,165,57,0.4)" }}>418</span>
              <span className="text-[10px] uppercase" style={{ letterSpacing: "0.12em", color: "var(--rm-text-faint)" }}>records</span>
            </div>
            <div className="h-[10px] rounded-full overflow-hidden flex mb-3" style={{ background: "rgba(255,255,255,0.07)" }}>
              {recordsByStatus.map((d) => (
                <div key={d.status} style={{ width: `${(d.count / totalRecords) * 100}%`, background: d.color }} />
              ))}
            </div>
            <div className="space-y-2">
              {recordsByStatus.map((d) => (
                <div key={d.status} className="flex items-center justify-between text-[11px]">
                  ...
                  <span className="font-bold tabular-nums">{d.count} <span style={{ color: "var(--rm-text-faint)", fontWeight: 500 }}>· {((d.count / totalRecords) * 100).toFixed(1)}%</span></span>
                </div>
              ))}
            </div>
          </Glass>
          ...
          <Glass className="col-span-3 px-5 py-4">
            <SectionLabel right="active only" takeaway="Construction Mgmt carries the biggest share of live projects.">Projects by Division</SectionLabel>
            <div className="space-y-2">
              {projectsByDivision.map((d) => (
                <div key={d.div} className="flex items-center gap-2 text-[10.5px]">
                  <span className="w-[104px] shrink-0 leading-tight" style={{ color: "var(--rm-text-muted)" }}>{d.div}</span>
                  <div className="flex-1 h-[8px] rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}>
                    <div className="h-full rounded-full" style={{
                      width: `${(d.count / maxDivCount) * 100}%`,
                      background: "linear-gradient(90deg, rgba(107,165,57,0.55), #8EC94A)",
                      boxShadow: "0 0 8px rgba(107,165,57,0.4)",
                    }} />
                  </div>
                  <span className="w-6 text-right font-bold tabular-nums" style={{ color: "#A8D672" }}>{d.count}</span>
                </div>
              ))}
            </div>
          </Glass>
          ...
          <Glass className="col-span-3 px-5 py-4 flex flex-col">
            <SectionLabel right="% of decided pursuits" takeaway="Our win rate keeps improving — best quarter yet.">Win Rate Trend</SectionLabel>
          ...
          <Glass className="col-span-3 px-5 py-4">
            <SectionLabel right="214 active projects" takeaway="8 in 10 projects are running on time.">Schedule Health</SectionLabel>
            <div className="h-[10px] rounded-full overflow-hidden flex mb-3" style={{ background: "rgba(255,255,255,0.07)" }}>
              {health.map((h) => (
        ...
        <Glass className="px-5 py-4">
          <SectionLabel right="backlog $M · deployed staff · health" takeaway="Two divisions carry nearly two-thirds of the backlog.">Division Scorecard</SectionLabel>
