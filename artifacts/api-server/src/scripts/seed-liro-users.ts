// Seed login users for the LiRo tenant directly on the prod AWS SQL Server.
// Mirrors lib/staff.ts createStaffRds (insertUser into dbo.rmone_users) but with
// a known password instead of the scrambled-hash + invite flow, per client request.
// Run: npx tsx src/scripts/seed-liro-users.ts
...
const TID = uuidv5("liro", TENANT_NAMESPACE);
// Never hardcode the password in the repo — pass it at run time:
//   SEED_USER_PASSWORD='...' npx tsx src/scripts/seed-liro-users.ts
...
const USERS: SeedUser[] = [
  { username: "AlexCarter92",  role: "CEO",               email: "ceo@liro.com",      access: "admin" },
  { username: "MorganBlake",   role: "CFO",               email: "cfo@liro.com",      access: "admin" },
  { username: "RileyStone",    role: "COO",               email: "coo@liro.com",      access: "admin" },
  { username: "SilverHawk76",  role: "Project Manager",   email: "pm@liro.com",       access: "manager" },
  { username: "EthanCole01",   role: "Resource Manager",  email: "rm@liro.com",       access: "manager" },
  { username: "OliviaReed02",  role: "Project Executive", email: "pe@liro.com",       access: "manager" },
  { username: "testuser",      role: "User",              email: "testuser@liro.com", access: "user" },
];
...
async function main() {
  console.log("LiRo tenant id:", TID);
  ...
  console.log("done — 7 users created for tenant 'liro'");
  process.exit(0);
}
